...
def test_pattern_flat():
    ...
    assert Required(NotRequired(OptionsShortcut()), NotRequired(Option("-a", None))).flat(
        OptionsShortcut
    ) == [OptionsShortcut()]
...
def test_parse_pattern():
    ...
    assert parse_pattern("( -h | -v [ --file <f> ] )", options=o) == Required(
        Required(
            Either(
                Option("-h"),
                Required(Option("-v", "--verbose"), NotRequired(Option("-f", "--file", 1, None)),),
            )
        )
    )
        ...
        Required(
            Either(
                Required(
                    Argument("N"),
                    NotRequired(
                        Either(Argument("M"), Required(Either(Argument("K"), Argument("L"))),)
                    ),
                ),
